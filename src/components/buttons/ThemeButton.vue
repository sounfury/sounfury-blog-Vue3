<template>
  <label class="theme-switch">
    <input type="checkbox" class="theme-switch__checkbox" ref="themeSwitch" @click="toggleTheme" />
...
//流程：亮旧页面（z-index 1）->暗旧页面（z-index 1000000）->暗新页面（z-index 1）->亮新页面（z-index 1000000）
...
function fallbackAnimation(event) {
  ...
  animationLayer.className = "theme-animation";
  ...
  // 动画结束后移除元素
  animationLayer.addEventListener("animationend", () => {
...
.theme-animation {
  ...
  width: 100vw;
  height: 100vh;
  animation: expand-circle 0.3s ease-out forwards;
  pointer-events: none;
...
.theme-switch {
  ...
  --container-light-bg: rgb(61, 126, 174, 0.7);
  --container-night-bg: rgb(29, 31, 44, 0.7);
  --circle-container-diameter: 3.375em;
  /* 光晕大小 */
  --sun-moon-diameter: 1.725em;
  /* 修改月球或太阳大小 */
  --sun-bg: #ecca2f;
  --moon-bg: #c4c9d1;
  --spot-color: #959db1;
